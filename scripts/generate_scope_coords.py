...
def load_domains(scopedom_filepath, scope_pdb_dir):
    ...
    remove_idxs = [
        idx for idx, domain in enumerate(domains) if domain["domain_id"] in IGNORE_DOMAIN_IDS
    ]
    remove(domains, remove_idxs)
    print(f"After ignoring problematic domains: {len(domains)}")

    # update domains from pdb files
    remove_idxs = []
    for idx, domain in enumerate(tqdm(domains)):
        scope_pdb_filepath = os.path.join(
            scope_pdb_dir, domain["domain_id"][2:4], "{}.ent".format(domain["domain_id"])
        )
    ...
    remove(domains, remove_idxs)
    print(f"After removing domains with missing pdb information: {len(domains)}")
    ...
    for idx, domain in enumerate(domains):
        if any([model["missing"] for model in domain["models"]]):
            remove_idxs.append(idx)
    remove(domains, remove_idxs)
    print(f"After removing domains with unknown residues: {len(domains)}")
    ...
    for idx, domain in enumerate(domains):
        if any([model["mismatched"] for model in domain["models"]]):
            remove_idxs.append(idx)
    remove(domains, remove_idxs)
    print(f"After removing domains with mismatched sequences: {len(domains)}")
    ...
    for idx, domain in enumerate(domains):
        if any([model["multisegmented"] for model in domain["models"]]):
            remove_idxs.append(idx)
    remove(domains, remove_idxs)
    print(f"After removing domains with multiple segments: {len(domains)}")
    ...
    for idx, domain in enumerate(domains):
        if any([model["backbone_coords"] is None for model in domain["models"]]):
            remove_idxs.append(idx)
    remove(domains, remove_idxs)
    print(f"After removing domains with missing atoms: {len(domains)}")
    ...
    with open(filepath) as file:
        for line in file:
            if line[0] == ">":
                ...
                domains.append({"domain_id": domain_id, "segments": segments, "full_sequence": ""})
            ...
            if row_type == "REMARK":
                ...
                for atom in atoms:
                    ...
                    if atom["alt_loc"] is None:
                        ...
                        residues[-1]["atoms"][atom_name]["alt_locs"][atom["alt_loc"]] = atom[
                            "coord"
                        ]
                ...
                for remark_segment_idx in range(len(remarks["Segments"])):
                    chain_id, start_res_seq, start_ins_code, end_res_seq, end_ins_code = remarks[
                        "Segments"
                    ][remark_segment_idx]
                    ...
                    assert (
                        domain["segments"][remark_segment_idx]["start_ins_code"] == start_ins_code
                    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-o", "--output_dir", type=str, help="Output directory", default=DEFAULT_OUTPUT_DIR
    )
    ...
    if not os.path.exists(scopedom_filepath):
        print(f"Missing SCOPe Domain File: {scopedom_filepath}")
        exit(0)
    elif not os.path.exists(scope_pdb_dir):
        print(f"Missing SCOPe PDB Directory: {scope_pdb_dir}")
        exit(0)
    elif os.path.exists(output_dir):
        print(f"Output Directory Existed: {output_dir}")
        exit(0)
